...
// ParseNameFromPath parses forms of a URL path into a Name. Specifically,
// it trims any leading "/" and then calls [ParseName] with fill.
func ParseNameFromPath(s, fill string) Name {
	s = strings.TrimPrefix(s, "/")
	return ParseName(s, fill)
}

// Path returns a complete, canonicalized, relative URL path using the parts of a
// complete Name.
//
// The parts maintain their original case.
//
// It panics if r is incomplete. Callers should check [Name.IsComplete] before
// calling this method.
//
// Example:
//
//	ParseName("example.com/namespace/model:tag+build").Path() // returns "/example.com/namespace/model:tag+build"
func (r Name) Path() string {
	if !r.IsComplete() {
